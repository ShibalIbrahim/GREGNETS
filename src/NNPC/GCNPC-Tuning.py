...
parser = argparse.ArgumentParser(description='GCN with partial correlation using Knowledge Graph.')
...
args.gcn_preestimator_params = {'n_steps': 65, 'optimizer': 'adam', 'lr': 0.001, 
                                'batch_size': 64, 'seed': 10,
                                'num_layers': 1, 'units': 1, 'activation': None,
                                'output_activation': 'linear', 'use_bias': True,
                                'kernel_regularizer': None, 'model_type': 'gcn'}
    
log_folder = """MODEL{}_COHORT{}_TS{}_REG{}_KG{}_MASK{}_Y{}_NTRIALS{}_TUNING{}_V{}""".format(
    'GCNPC', args.cohort, args.time_series, args.regularizer, args.knowledge_graph, args.mask, args.num_training_years, args.ntrials, args.tuning_metric, args.version)
...
supports_train = graph_utils.create_supports([cf_train], model_type=args.gcn_preestimator_params['model_type'])
...
X, Y = data_utils.prepare_sequences(df_train.values, args.gcn_preestimator_params['n_steps'])
X_val, Y_val = data_utils.prepare_sequences(np.append(df_train.values[-args.gcn_preestimator_params['n_steps']:],df_val.values, axis=0), args.gcn_preestimator_params['n_steps'])
X_test, Y_test = data_utils.prepare_sequences(np.append(df_val.values[-args.gcn_preestimator_params['n_steps']:],df_test.values, axis=0), args.gcn_preestimator_params['n_steps'])
...
model_hat = models.create_gcn_model(
    input_shape=X.shape[1:],
    units=[*[args.gcn_preestimator_params['units']]*(args.gcn_preestimator_params['num_layers']-1), 1],
    activation=args.gcn_preestimator_params['activation'],
    output_activation=args.gcn_preestimator_params['output_activation'],
    supports=supports_train,
    use_bias=args.gcn_preestimator_params['use_bias'],
    kernel_regularizer=args.gcn_preestimator_params['kernel_regularizer'],
    kernel_initializer=tf.keras.initializers.GlorotUniform(seed=args.gcn_preestimator_params['seed']),
)
model_hat.save_weights(os.path.join(path, 'model_initial.h5'), overwrite=True)
model_hat.compile(loss='mean_squared_error', optimizer=get_optimizer(args.gcn_preestimator_params['optimizer'], args.gcn_preestimator_params['lr']), metrics=['mse'])
model_hat.fit(X, 
              Y, 
              epochs=args.epochs, 
              batch_size=args.gcn_preestimator_params['batch_size'], 
              shuffle=True,
              callbacks=callbacks,
...
space = {'regularizer': hp.choice('regularizer', ['l2']),
         'lambda': hp.choice('lambda', np.logspace(start=-7, stop=-3, num=10, base=10)),
         'batch_size' : hp.choice('batch_size', [32, 64, 128, 256]),
         'epochs' :  args.epochs,
         'optimizer': hp.choice('optimizer',['adam']),
         'lr': hp.choice('lr', np.logspace(start=-4, stop=-2, num=20, base=10)),
         'n_steps' : hp.choice('n_steps', [40, 50, 60, 65, 70, 75, 80]),
...
def f_model(params):
    ...
    model = models.create_model_with_pseudolikelihood(
        models.create_gcn_model,
        {'input_shape': X.shape[1:],
         'units': [*[params['units']]*(params['num_layers']-1), 1],
         'activation': params['activation'], 'output_activation': params['output_activation'],
         'supports': supports_train,
         'kernel_regularizer': tf.keras.regularizers.l2(params['lambda']),
         'kernel_initializer': tf.keras.initializers.GlorotUniform(seed=params['seed_initializer'])},
...
supports_val = graph_utils.create_supports([cf_val], model_type=args.gcn_preestimator_params['model_type'])
...
model_opt = models.create_model_with_pseudolikelihood(
    ...
    {'input_shape': X_train_val.shape[1:],
     'units': [*[best_run['units']]*(best_run['num_layers']-1), 1],
     'activation': best_run['activation'], 'output_activation': best_run['output_activation'],
     'supports': supports_val,
     'kernel_regularizer': tf.keras.regularizers.l2(best_run['lambda']),
     'kernel_initializer': tf.keras.initializers.GlorotUniform(seed=best_run['seed_initializer'])},
