...
log_folder = """MODEL{}_COHORT{}_TS{}_REG{}_KG{}_MASK{}_LEVEL{}_Y{}_NSTEPS{}_V{}""".format(
    'VARPC', args.cohort, args.time_series, args.regularizer, args.knowledge_graph, args.KG_mask, args.mask_sparsity, args.num_training_years, args.n_steps, args.version)
...
varpc = models.VARPC(cf_train, args.regularizer, args.KG_mask, args.mask_sparsity, path=path, convergence_tolerance=args.convergence_tolerance, max_iter=args.max_iter, warm_start=True)
...
if args.time_series == 'volume':
    ...
    if args.regularizer == 'Lasso':
        if args.KG_mask is None:
            lams_rho = np.logspace(start=-4, stop=-7, num=25, base=10.0)
        elif args.KG_mask == 'hard':
            lams_rho = np.logspace(start=-4, stop=-7, num=25, base=10.0)
        elif args.KG_mask == 'soft':
            lams_rho = np.logspace(start=-5, stop=-8, num=25, base=10.0)
        ...
        if args.KG_mask is None:
            ...
            lams_rho = np.logspace(start=-7, stop=-10, num=25, base=10.0)
elif args.time_series == 'volatilities':
    lams_A = np.logspace(start=-4, stop=-7, num=20, base=10.0)
    if args.regularizer == 'Lasso':
        if args.KG_mask is None:
            lams_rho = np.logspace(start=-3, stop=-6, num=25, base=10.0)
        elif args.KG_mask == 'hard':
            lams_rho = np.logspace(start=-4, stop=-7, num=25, base=10.0)
        elif args.KG_mask == 'soft':
            lams_rho = np.logspace(start=-4, stop=-7, num=25, base=10.0)
        ...
        if args.KG_mask is None:
            ...
            lams_rho = np.logspace(start=-6, stop=-9, num=25, base=10.0)

T, N, p = X.shape
val_opt = np.inf
with open(os.path.join(path, args.hp_tuning_file), 'w') as f:
    with redirect_stdout(f):
        print('{0}  {1}  {2}  {3}  {4}  {5}'.format('lambda_A', 'lambda_rho', 'MSE', 'MSE-val', 'R^2-val', 'Sparsity'))
...
for lambda_A in notebook.tqdm(lams_A, 'lambda_A'):
    for lambda_rho in notebook.tqdm(lams_rho, 'lambda_rho'):
        ...
        varpc.fit(X, Y, lambda_A, lambda_rho, args.learning_rate)
#         varpc.warm_starts = True
        MSE, _, sparsity = varpc.evaluate(X, Y)
        MSE_val, Rsquared_val, _ = varpc.evaluate(X_val, Y_val)
        val = deepcopy(MSE_val)
        with open(os.path.join(path, args.hp_tuning_file), 'a') as f:
            with redirect_stdout(f):
                print('{0:.12f}  {1:.12f}  {2:.5f}  {3:.5f}  {4:.5f}  {5:.2f}'.format(lambda_A, lambda_rho, MSE, MSE_val, Rsquared_val, sparsity))      
        print('{0:.12f}  {1:.12f}  {2:.5f}  {3:.5f}  {4:.5f}  {5:.2f}'.format(lambda_A, lambda_rho, MSE, MSE_val, Rsquared_val, sparsity))      
...
with open(os.path.join(path, args.results_file), 'w') as f:
    with redirect_stdout(f):
        print('Optimal: lambda_A:{:.12f}\n lambda_rho:{}\n MSE-val:{:.5f}\n R2-val:{:.5f}, sparsity:{:.2f}\n\n'.format(lambda_A_opt, lambda_rho_opt, MSE_val_opt, Rsquared_val_opt, sparsity_opt)) 
display(Math(r'~Optimal: \lambda_A:{:.12f},~\lambda_\rho:{},~MSE-val:{:.5f},~R^2-val:{:.5f},~sparsity:{:.2f}'.format(lambda_A_opt, lambda_rho_opt, MSE_val_opt, Rsquared_val_opt, sparsity_opt)))
...
varpc_final = models.VARPC(cf_val, args.regularizer, args.KG_mask, args.mask_sparsity, path=path, max_iter=args.max_iter, convergence_tolerance=args.convergence_tolerance, warm_start=False)
...
start = time.time()
varpc_final.fit(X_train_val, Y_train_val, lambda_A_opt, lambda_rho_opt, args.learning_rate)
end = time.time()

MSE_train_val, Rsquared_train_val, sparsity_final = varpc_final.evaluate(X_train_val, Y_train_val)
MSE_test, Rsquared_test, _ = varpc_final.evaluate(X_test, Y_test)

with open(os.path.join(path, args.results_file), 'a') as f:
    with redirect_stdout(f):
        print('MSE-test:{:.5f}\n R2-test:{:.5f}\n Sparsity:{:.2f}\n'.format(MSE_test, Rsquared_test, sparsity_final))
display(Math(r'~Optimal: \lambda_A:{:.8f},~\lambda_\rho:{},~MSE:{:.5f},~R^2:{:.5f},~MSE-test:{:.5f},~R^2-test:{:.5f},~sparsity:{:.2f}'.format(lambda_A_opt, lambda_rho_opt, MSE_train_val, Rsquared_train_val, MSE_test, Rsquared_test, sparsity_final)))
...
rho_final = varpc_final.rho_
c_final = varpc_final.c_
A_final = varpc_final.A_
...
# MSE_test, Rsquared_test, _ = varpc_opt.evaluate(X_test, Y_test)

# with open(os.path.join(path, args.results_file), 'a') as f:
#     with redirect_stdout(f):
#         print('MSE-test:{:.5f}\n R2-test:{:.5f}\n Sparsity:{:.2f}\n'.format(MSE_test, Rsquared_test, sparsity_opt))
# display(Math(r'~MSE-test:{:.5f},~R^2-test:{:.5f},~sparsity:{:.2f}'.format(MSE_test, Rsquared_test, sparsity_opt)))

# ### Visualize partial correlation
# rho_opt = varpc_opt.rho_
# c_opt = varpc_opt.c_
# A_opt = varpc_opt.A_

# ## Save optimal model
# with open(os.path.join(path, 'model.pkl'), 'wb') as output:
#     dill.dump(varpc_opt, output)

# ## Save results
# np.savez_compressed(
#     os.path.join(path, 'optimal_solution'),
#     lambda_A=lambda_A_opt,
#     lambda_rho=lambda_rho_opt,
#     A=A_opt,
#     rho=rho_opt,
